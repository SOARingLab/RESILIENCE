...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "engine-jbpm-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name engine-jbpm-service apps/engine-jbpm-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app engine-jbpm-service:1.0-SNAPSHOT
    oc expose svc/engine-jbpm-service
else

	echo "Launching the application locally..."
	pattern="engine-jbpm-service"
	files=( $pattern )
	cd ${files[0]}
